...
public class SaleQueries {
  ...
  public static String selectSalesByCustomerReportQuery =
      "SELECT u.id AS cliente_id, u.nome AS cliente_nome, COUNT(v.id) as qtde_compras FROM venda v JOIN usuario u ON v.usuario_id = u.id WHERE v.data_hora BETWEEN CAST(? AS timestamp) AND CAST(? AS timestamp) GROUP BY u.id, u.nome ORDER BY qtde_compras DESC";
  ...
  public static String selectDailySalesReportQuery =
      "SELECT DATE(v.data_hora) AS venda_data, "
          ...
          + "WHERE v.data_hora BETWEEN CAST(? AS timestamp) AND CAST(? AS timestamp) "
