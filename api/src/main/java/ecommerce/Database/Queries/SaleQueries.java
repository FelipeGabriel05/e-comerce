...
public class SaleQueries {
  ...
  public static String selectSaleItemsBySaleIdQuery =
      "SELECT vp.produto_id, vp.preco, vp.quantidade, p.descricao, p.foto, p.categoria_id FROM venda_produto vp JOIN produto p ON vp.produto_id = p.id WHERE vp.venda_id = ?";
